...
export const QRScannerModal: React.FC<QRScannerModalProps> = ({
  ...
  // Start Camera Stream with Progressive Fallbacks for Android WebView & Mobile
  const startCamera = async (mode: 'environment' | 'user') => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      setCameraError('Camera access is not supported by your browser or inside this frame. Khawngaihin "Snap with Camera" emaw Gallery hmang rawh le.');
      setIsStartingCamera(false);
      return;
    }

    // Try progressive fallback constraint configurations
    ...
    for (const constraints of constraintList) {
      try {
        ...
        lastErr = err;
        // If user actively denied permission, break immediately
    ...
    if (!stream) {
      console.warn('All camera constraints failed:', lastErr);
      setCameraError(
        lastErr?.name === 'NotAllowedError' || lastErr?.name === 'PermissionDeniedError'
          ? 'Camera permission was denied. Khawngaihin Camera permission pe la emaw "Snap with Camera" hmang rawh le.'
          : 'Camera stream a in-start thei lo. "Snap with Camera" emaw Gallery thlalak hmang rawh le.'
      );
      setCameraActive(false);
      setIsStartingCamera(false);
      return;
      ...
      try {
        ...
        setTimeout(async () => {
          try {
            ...
            setCameraError('Camera stream render failed. Khawngaihin "Snap with Camera" hmang rawh le.');
            setCameraActive(false);
            setIsStartingCamera(false);
          }
        }, 300);
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/95 z-50 flex flex-col justify-between p-3 sm:p-4 backdrop-blur-md animate-fadeIn text-white overflow-y-auto">
      ...
      {/* Hidden native direct camera capture input (100% Android WebView compatible) */}
      <input 
        type="file" 
      ...
      <div className="relative my-auto flex flex-col items-center justify-center py-2 shrink-0">
        <div className="relative w-64 h-64 sm:w-72 sm:h-72 border-2 border-amber-400/80 rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(251,191,36,0.35)] bg-slate-900 flex items-center justify-center">
          ...
          {!cameraActive && (
            <div className="text-center p-4 space-y-2.5">
              ...
              <p className="text-xs font-bold text-slate-200 max-w-[200px] mx-auto leading-tight">
                {cameraError || (isStartingCamera ? 'Camera stream on mek a ni...' : 'Camera stream in-start nghah mek a ni')}
              </p>
              ...
              <div className="flex flex-col gap-2 pt-1">
                ...
                >
                  <Camera className="w-3.5 h-3.5" /> Snap with Native Camera
                </button>
              </div>
            </div>
          )}
        ...
        <div className="mt-3 flex items-center gap-2 flex-wrap justify-center">
          <button
            onClick={() => cameraCaptureInputRef.current?.click()}
            className="text-xs text-emerald-300 font-bold bg-emerald-950/70 border border-emerald-500/40 px-3 py-1.5 rounded-full flex items-center gap-1.5 hover:bg-emerald-900/80 transition cursor-pointer"
          >
            <Camera className="w-3.5 h-3.5 text-emerald-400" /> Snap Photo
          </button>
          <button
            onClick={() => fileInputRef.current?.click()}
            className="text-xs text-amber-300 font-bold bg-amber-950/60 border border-amber-500/40 px-3 py-1.5 rounded-full flex items-center gap-1.5 hover:bg-amber-900/80 transition cursor-pointer"
          >
