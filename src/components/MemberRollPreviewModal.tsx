...
export const MemberRollPreviewModal: React.FC<MemberRollPreviewModalProps> = ({
  ...
  const allowedCampaigns = useMemo(() => {
    ...
    if (isPrivilegedUser) {
      return kumtluangCampaigns.filter(c => {
        ...
        const count = members.filter(m => m.campaignId === c.id || (c.orgCode && m.orgCode === c.orgCode)).length;
    ...
    return kumtluangCampaigns.filter(c => isCampaignCreator(c, creatorProfile));
  }, [campaigns, creatorProfile, isPrivilegedUser, members, transactions]);
  ...
  React.useEffect(() => {
    if (isOpen) {
      ...
      if (initialCampaignId && (initialCampaignId === 'all' ? (isPrivilegedUser || allowedCampaigns.length > 1) : allowedCampaignIds.has(initialCampaignId))) {
  ...
  }, [isOpen, initialFormat, initialCampaignId, initialMemberId, allowedCampaigns, allowedCampaignIds, isPrivilegedUser]);
  ...
  const scopedMembers = useMemo(() => {
    if (allowedCampaigns.length === 0 && !isPrivilegedUser) return [];
    
    // Base filter by allowed campaigns
    const allowedList = isPrivilegedUser ? members : members.filter(m => {
  ...
  }, [members, selectedCampaignId, activeCampaign, allowedCampaigns.length, allowedCampaignIds, allowedOrgCodes, isPrivilegedUser]);

  // Scoped transactions
  const scopedTransactions = useMemo(() => {
    if (allowedCampaigns.length === 0 && !isPrivilegedUser) return [];
    
    const allowedTx = isPrivilegedUser ? transactions : transactions.filter(t => allowedCampaignIds.has(t.campaignId));
  ...
  }, [transactions, selectedCampaignId, activeCampaign, allowedCampaigns.length, allowedCampaignIds, isPrivilegedUser]);
  ...
  const handleDownloadHtml = () => {
    const html = generateReportHtml();
    const fileName = `${orgTitle.replace(/\s+/g, '_')}_MemberRoll_${selectedFormat}.html`;
    downloadFileUniversal(html, fileName, 'text/html;charset=utf-8');
  };
  ...
  return (
    <div className="fixed inset-0 z-60 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-0 sm:p-4 animate-fadeIn text-slate-900">
      <div className="bg-white border-0 sm:border border-slate-200 sm:rounded-3xl rounded-none w-full max-w-6xl shadow-2xl overflow-hidden flex flex-col h-full sm:h-[90vh] max-h-[90vh] shrink-0">
        
        {/* TOP MODAL TOOLBAR */}
        <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-indigo-950 px-4 sm:px-6 py-3.5 border-b border-slate-800 text-white flex flex-col md:flex-row items-start md:items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-[9px] font-black uppercase px-2 py-0.5 rounded-md shrink-0">
                  Live View
                </span>
              </div>
              <p className="text-[11.5px] text-indigo-200/80 truncate">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-end shrink-0">
            <div className="flex items-center gap-1.5">
              <button
                type="button"
                onClick={handlePrint}
                className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-black flex items-center gap-1.5 shadow-md transition cursor-pointer active:scale-95"
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-2 rounded-xl bg-white/10 hover:bg-white/20 text-slate-200 hover:text-white transition cursor-pointer shrink-0"
              title="Close Preview"
            >
              <X className="w-5 h-5" />
            </button>
        ...
        {/* CONTROLS & FILTER BAR */}
        <div className="bg-indigo-50/80 px-4 sm:px-6 py-3 border-b border-indigo-100 flex flex-wrap items-center justify-between gap-3 shrink-0 text-xs">
          
          {/* Left Controls: Format & QR Scope */}
          <div className="flex flex-wrap items-center gap-2.5">
            {/* Format Selector */}
            <div className="flex items-center gap-1.5">
              <label htmlFor="preview-format-select" className="text-slate-700 font-black shrink-0">
                Format:
              </label>
              <select
                ...
                className="bg-white border border-indigo-200 text-slate-900 font-bold rounded-xl px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-2xs cursor-pointer"
              >
            ...
            {/* Bawm Scope Selector */}
            <div className="flex items-center gap-1.5">
              <label htmlFor="preview-camp-select" className="text-slate-700 font-black shrink-0">
                QR Bawm:
              </label>
              <select
            ...
            {availableSections.length > 0 && selectedFormat === 'style1_master' && (
              <div className="flex items-center gap-1.5">
                <label htmlFor="preview-sec-select" className="text-slate-700 font-black shrink-0">
                  Section:
                </label>
                <select
                  ...
                  className="bg-white border border-indigo-200 text-slate-900 font-bold rounded-xl px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-2xs cursor-pointer max-w-[150px] truncate"
                >
            ...
            {(selectedFormat === 'style2_matrix' || selectedFormat === 'style3_passbook') && (
              <div className="flex items-center gap-1.5">
                <label htmlFor="preview-member-select" className="text-slate-700 font-black shrink-0">
                  Member:
                </label>
                <select
                  ...
                  className="bg-white border border-indigo-200 text-slate-900 font-bold rounded-xl px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-2xs cursor-pointer max-w-[180px] truncate"
                >
                  {scopedMembers.map(m => (
          ...
          {selectedFormat === 'style1_master' && (
            <div className="relative w-full sm:w-60">
              ...
              <input
                ...
                className="w-full pl-8.5 pr-7 py-1.5 bg-white border border-indigo-200 rounded-xl text-xs font-bold text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-2xs"
              />
              {searchQuery && (
                <button
                  type="button"
        ...
        <div className="flex-1 min-h-0 bg-slate-100 p-3 sm:p-6 overflow-y-auto overflow-x-auto flex justify-center items-start">
